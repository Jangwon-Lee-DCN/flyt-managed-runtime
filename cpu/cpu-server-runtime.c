...
bool_t cuda_event_elapsed_time_1_svc(ptr start, ptr end, float_result *result, struct svc_req *rqstp)
{
    ...
    if (resource_mg_get(&rm_events, (void*)start, (void**)&start_ptr) != 0) {
    ...
    cudaEvent_t end_ptr;
    if (resource_mg_get(&rm_events, (void*)end, (void**)&end_ptr) != 0) {
...
bool_t cuda_malloc_pitch_1_svc(size_t width, size_t height, ptrsz_result *result, struct svc_req *rqstp)
{
    ...
    if (result->err == cudaSuccess) {
        mem_alloc_args_t *args = malloc(sizeof(mem_alloc_args_t));
        args->type = MEM_ALLOC_TYPE_PITCH;
        args->width = width;
        args->height = height;
...
bool_t cuda_memcpy_to_symbol_1_svc(uint64_t symbolptr, mem_data mem, size_t size, size_t offset, int *result, struct svc_req *rqstp)
{
    ...
    GET_CLIENT(*result)
    GET_VARIABLE(symbol_addr, symbolptr, *result)
